...
**MLB home-run props — already-built framework (chat.ts SYSTEM_PROMPT ~lines 195-202).** When a user hands over a Statcast HR framework, the supportable subset is ALREADY wired and enumerated in the HR section + NEVER-FABRICATE clause. Map their list:
- SUPPORTED (real fields): park hrIndex + altitudeFt + dome (MLB_PARKS static), weather tempF (temperature is the reliable signal), opposing starter tendency hrPer9 / flyBallPct / oppOPS (ESPN, PITCHER's stat), mlbPlatoon handedness + real vsThatHand splits, playerHistory recent HR/XBH form, run environment (implied team total derived from total+spread).
- NOT SUPPORTED → in NEVER-FABRICATE list (no feed; banning is the whole point): exit velocity, launch angle, barrel rate, hard-hit %, batter fly-ball rate, batter pull rate / spray direction, pitch-type/mix, live pitch count / in-game fatigue / velo drop, day-vs-night HR split, batter-vs-this-pitcher career line.
- **Wind gotcha:** weather only captures `wind.speed` (windMph) — NEVER `wind.deg`. So the AI must NOT claim wind is "blowing out/in"; treat high windMph only as an amplifier of the temp read. Same for "day vs night": no clean venue-local first-pitch time, so don't infer it.
**Why:** the user listing a metric as a "factor they want" is itself a fabrication trigger — the never-fabricate clause is reworded to resist exactly that. **How to apply:** for any new "factor in X" request, check it against this map before touching the prompt; only enumerate fields that exist in the live context payload.
