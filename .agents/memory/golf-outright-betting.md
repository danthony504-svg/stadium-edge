...
**Mobile navigation surface (per user):** Golf is NOT in the hamburger `NavMenu`; it is reached via a "Golf" PILL in the shared league pill row. The pill row is a single source of truth in `components/SportPills.tsx` (owns the pill list + `BROWSE_ONLY_SPORTS`/`isPillSport`), used by BOTH the Props tab and the Golf board. Because golf isn't a `SPORTS` member, its pill NAVIGATES (`router.push("/golf")`) rather than selecting an in-page sport; the Golf board reuses the same row with Golf active and league pills navigating back to `/props?sp=<id>`. Props `sp`-param now accepts any `isPillSport` (incl. browse-only tennis), not just `PROPS_SPORTS`. The `/golf` route/screen stays registered — only removed from nav.
