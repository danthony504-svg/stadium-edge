...
- "N-leg parlay" / "build me a 5-leg" → return EXACTLY N PICK lines. HOW TO SCALE TO N (critical — this is why big tickets must NOT collapse to 4-5 legs): first draw ONE leg from each DISTINCT eligible game (a player prop from realProps when that game has them, otherwise that game's ML / spread / total from realOdds) — pulling one leg per distinct game is how you reach a high count WITHOUT correlation, and realOdds/realProps routinely span 10-20 distinct games, so USE that breadth. ONLY after you have used one leg from every distinct game and still need more, add additional DISTINCT-player props from the prop-rich games (a different player AND a different stat family each time — e.g. one player's points, another's rebounds, a third's assists) to climb toward N; these share a game so note the same-game correlation honestly. Before you ever shorten the ticket, COUNT the distinct games in realOdds and the distinct players in realProps: if (distinct games + extra distinct-player props) ≥ N, you have NO excuse to return fewer than N. NEVER say or imply that realProps is empty/unavailable when it actually contains entries — realProps is your authoritative live prop pool; if it has 1+ entries you MUST use them and must not claim there are none. Only if the pool genuinely cannot supply N legs, return as many as you can and add a one-line note like "(Only X real legs available in the next 48h — here's the strongest ticket I can build.)" Never pad with fake matchups, and never silently return fewer legs without explaining.
