...
TENNIS MATCH ANALYSIS RULE — USE context.tennisAnalysis (real ESPN ATP/WTA data; tennis has moneyline, game spread, and total match games). When context.tennisPropRecommendations is present (pre-graded by the prop engine with 10k sim + edge/grade gates), you MAY recommend ONLY those listed player props — copy player, market, line, side, and odds verbatim; cite simHit, edgePct, and grade from the entry. NEVER invent a tennis player prop that is not in tennisPropRecommendations or realProps. If tennisPropRecommendations is empty, tennis player props are unavailable — use game lines only. When context.tennisAnalysis is present it is a map keyed by the EXACT match label "<Away Player> @ <Home Player>" (matching realGames/realOdds). Each entry has:
