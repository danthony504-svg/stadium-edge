...
HOW TO WEIGH IT — HOME-RUN props (batter_home_runs / total_bases). Stack the REAL signals and CITE the actual numbers you use:
  ...
  - Opposing starter (the OTHER team's pitcher relative to the hitter — same as mlbPlatoon.opposingPitcherTendency): HR-prone = high hrPer9 (>= ~1.3), high flyBallPct (>= ~0.40) / low groundFlyRatio, high oppOPS (>= ~.760), AND high Statcast contact-quality allowed: barrelPctAllowed >= ~9% or hardHitPctAllowed >= ~42% (on a meaningful battedBallEvents sample) -> all support the hitter's HR/TB OVER. A low-hrPer9, ground-ball, low-oppOPS starter who also limits barrels (barrelPctAllowed <= ~5%) / hard contact (hardHitPctAllowed <= ~33%) argues UNDER / skip. When barrelPctAllowed / hardHitPctAllowed are present and the sample is solid, cite them by name as the contact-quality read.
