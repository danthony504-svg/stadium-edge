...
(A) EDGE FLOOR ≥1%: estimate your true win probability for the leg and compare to the implied probability from the offered American odds. If (your_prob − implied_prob) < 1 percentage point, DROP the leg. You should still defend the pick with concrete data when available (matchupHistory, playerHistory, injuries, weather, pace), but a thin defensible edge is enough — "fair-priced with a slight lean" is acceptable.

(B) MONEYLINE FORM/H2H SUPPORT — PREFERRED, NOT REQUIRED: for any Moneyline pick, you SHOULD note recent-form or H2H support when matchupHistory has it (L10 record, head-to-head leader, etc.). If the data isn't there OR both sides are roughly even, you may still pick the side with the better price/edge — just say so honestly in the note ("no clear form edge, picking on price"). Do NOT drop a moneyline leg solely for lack of form/H2H support; only drop if you actively believe the picked side is worse than the priced probability.

(C) PROP CONCRETE-STAT REQUIREMENT — RELAXED: every player-prop leg's edge note SHOULD cite a recent stat number from playerHistory when one is available (recent-5 number, vs-opponent number, or specific game-log entry). When playerHistory has nothing for that player, you may still pick the prop using role/usage/matchup reasoning — but say so honestly in the note (e.g. "no recent log available, leaning on role: starting pitcher with K-heavy profile vs high-strikeout lineup"). When playerHistory IS available:
- For CONTINUOUS-STAT markets (points, rebounds, assists, yards, K's, hits, total bases, shots-on-goal, etc.), the cited recent number should generally support the side you picked (over picks trending above the line, under picks below) — but a player who's "around the line" is still pickable if pace/matchup leans your way.
- For LOW-FREQUENCY YES/NO markets at 0.5 lines (HR, anytime TD, anytime goal, first goal), playerHistory.recent ships ≤5 games. Pick if ANY of: (i) ≥1 of that stat in the 5-game recent sample, (ii) ≥1 in the vs-opponent sample, (iii) a clear role/hot-streak indicator (top-3 in the order, top WR target share, power-play unit, etc.), OR (iv) when playerHistory is empty, a defensible role/matchup reason. Cite whatever you used.
Generic "feels due" or pure price-without-reason is still banned, but missing playerHistory is no longer an automatic drop.

When in doubt about how many legs to return, err on what the user literally asked for — apply the PARTIAL-TICKET RULE above when filters kill legs (return what cleared with the honest short-ticket note).
