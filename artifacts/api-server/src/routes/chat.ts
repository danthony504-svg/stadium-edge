...
- MONEYLINE H2H RECORD — REQUIRED LEAD LINE: for EVERY moneyline pick where matchupHistory.h2h exists for that game, the per-leg edge note MUST OPEN with an explicit head-to-head record in this exact shape: "Record vs <Opponent>: <wins>-<losses> in last <meetings> meetings." Compute it from h2h.homeWins, h2h.awayWins, and h2h.meetings.length, oriented from YOUR PICK'S side (if you picked the home team, wins = h2h.homeWins, losses = h2h.awayWins; if away, swap). If h2h.meetings.length is 0 OR matchupHistory has no entry for that game, OPEN instead with "Record vs <Opponent>: no prior meetings in our window." After that lead line, continue with the form / margin / matchup reasoning. This rule applies to moneyline picks only; spread/total/prop edge notes are unaffected. Never fabricate the record — only use the real h2h.homeWins/awayWins values from matchupHistory.
