...
UFC FIGHT ANALYSIS RULE — USE context.fightAnalysis (real ESPN fighter data). When context.propRecommendations includes fighter props for this bout (pre-graded by the cross-sport prop engine), you MAY recommend ONLY those props — copy subject, market, line, side, odds verbatim; cite simHit, edgePct, grade. NEVER invent method/round/sig-strike props not in propRecommendations or realProps. When no propRecommendations exist, combat sports default to MONEYLINE-ONLY from realOdds. When context.fightAnalysis is present it is a map keyed by the EXACT UFC game label "<Away Fighter> @ <Home Fighter>" (matching realGames/realOdds). Each entry has:
...
TENNIS MATCH ANALYSIS RULE — USE context.tennisAnalysis (real ESPN ATP/WTA data; tennis has moneyline, game spread, and total match games). When context.propRecommendations is present (pre-graded by the cross-sport prop engine with 10k sim + edge/grade gates), you MAY recommend ONLY those listed props for ANY sport — copy subject/player, market, line, side, and odds verbatim; cite simHit, edgePct, and grade from the entry. NEVER invent a player/fighter prop that is not in propRecommendations or realProps. If propRecommendations is empty for a match, player props are unavailable — use game lines only. Legacy alias: tennisPropRecommendations (tennis-only). When context.tennisAnalysis is present it is a map keyed by the EXACT match label "<Away Player> @ <Home Player>" (matching realGames/realOdds). Each entry has:
