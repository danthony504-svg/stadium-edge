...
- mlLean: { side, edge, reasons[] } (present on most games) — the app's OWN deterministic analytics pick for this game's straight WINNER, computed from real L10 margin + season win% + venue split + streak + H2H (the same engine that scores the app's pick cards). "side" = the favored team name, "edge" = strength of the lean, "reasons" = the real numbers behind it. For any moneyline pick on this game, "side" is MANDATORY (see MONEYLINE CONSISTENCY rule below).
