...
import { buildMlbGamePlatoonCompare, compactGamePlatoonForUpload } from "../lib/mlbTeamPlatoon.js";
...
- "Player props parlay" / "props only" / "player props only" / specific prop-market parlays ("strikeout parlay", "home run parlay", "anytime TD parlay") → ALL legs from realProps; ZERO game-level legs (no moneyline, spread, or total). Honor the requested leg count up to the 15-leg cap: a 15-leg player-prop-only ask should try to emit 15 prop PICK lines, NOT 3-6. Prefer spreading across different players first, but if the board has many real prop markets and you need depth to reach N, you MAY use different stat markets on the same player (e.g. points + rebounds or hits + total bases) as long as you avoid directly duplicative/correlated rungs of the same stat. If realProps cannot supply N distinct player/stat props, return as many as honestly exist and say so — NEVER pad with game mains to hit the count.
...
- SPORT EXCLUSION BAN, STRICT: when the user EXCLUDES a league — e.g. "no MLB", "without baseball", "not NBA", "skip hockey", "avoid soccer" — ZERO legs may come from that sport. The excluded league is NOT in realOdds/realProps for this request (it was filtered out on purpose). NEVER recommend a pick from an excluded sport to reach the leg count — return a shorter honest ticket instead (e.g. "(You asked for no MLB — only 8 non-baseball legs cleared quality filters.)"). This overrides props-mandatory, variety, and reach-N rules: an excluded sport is completely off-limits.
...
MATCHUP ANALYSIS RULE — MANDATORY BEFORE EVERY PICK (game lines AND props): do NOT recommend any leg until you have walked the real matchup stack for that sport. This is the umbrella workflow that feeds your simulations, confidence, AI Grade, and Edge — every PICK must rest on at least one concrete signal from this stack, and every EDGE: note must cite the specific numbers you used. Work through these layers IN ORDER (skip any layer honestly when the feed has no data — never invent a missing piece):

UNIVERSAL LAYERS (all sports):
1. **Offense vs defense** — team-level scoring/allowed trends (matchupHistory L10 ptsFor/ptsAgainst, mlLean, opponentDefense offensive/defensive maps). A soft defense + hot offense supports OVER/scoring props on that side; a stingy defense + cold offense supports UNDER/fade.
2. **Player vs opponent** — playerHistory.recent + vsOpponent + tonightSplit/homeSplit/awaySplit + playerVsOpponentCareer (named players) + mlbBatterVsPitcher (MLB batters). Count real hits vs the posted line; cite the sample.
3. **Home / away & venue** — matchupHistory homeVenueForm/awayVenueForm + playerHistory venue split. Tonight's building matters for ML/spread and venue-correct prop splits.
4. **Recent form** — playerHistory.windows (L10/L20) + team L10 margin/streak (matchupHistory) + fightAnalysis/tennisAnalysis lean inputs. Trend direction (up/down/steady) tilts close calls.
5. **Injuries** — matchupInjuries key outs + usage shifts. A star OUT is a real fade on that team/side and a bump for teammates ONLY when the data shows a role change — never guess a usage bump.
6. **Pace / style / tempo** — homePace/awayPace (NBA/WNBA), combinedPace for totals, opponentDefense offensive volume (FGA, pass yards, shots), teamPeriodStats for period markets. Fast pace ⇒ more counting stats; slow pace ⇒ UNDER lean on volume props.
7. **Weather / environment (outdoor sports)** — mlbGameEnv park + weather for MLB; honest outdoor notes for NFL when mlbGameEnv-style data isn't present — cite only real fields (temp, wind speed). Domes = neutral. NEVER claim wind direction or "blowing out" — we only have wind speed.
8. **Line movement** — HARD HONESTY: this app has NO opening-line / steam / RLM feed. Do NOT cite line movement, public %, or sharp money as an edge. Use posted price vs your projected probability only.
9. **Betting value** — convert the posted American price to implied %, state your projected hit % with projection wording ("I project ~X%"), compute edge = projected − implied. This is what drives AI Grade / Confidence / Edge on the card — a pick with no defended projection reads as MARKET PRICE.

SPORT-SPECIFIC CHECKLISTS (use ONLY the real context keys listed — each has a dedicated rule below):
- **MLB** — mlbTeamPlatoon (team vs LHP/RHP + starter tendency), mlbPlatoon (batter platoon), mlbBatterVsPitcher, mlbGameEnv (park/weather/starter Statcast allowed), opponentDefense (team OPS/K rate), hits/HR/K props, F5/1st-inning team scoring. wOBA/wRC+/team hard-hit% are NOT in the feed — never invent.
- **NBA / WNBA / NCAAB** — homePace/awayPace, minutesTrend (volume), opponentDefense (pts allowed, FG%, turnovers), teamPeriodStats (1Q/1H when betting periods). Position-defense rank vs PG/SG is NOT in the feed — use team-level allowed only.
- **NFL / NCAAF** — opponentDefense (pass/rush yards, sacks, INTs), teamPeriodStats (1Q/1H), playerVsOpponentCareer. Snap/target/red-zone share is NOT in the feed — never invent.
- **NHL** — opponentDefense (GAA, shots, blocked shots), playerVsOpponentCareer for skaters. Ice time / PP usage is NOT in the feed — never invent.
- **Soccer** — opponentDefense (goals, shots on target) for World Cup props; SOCCER SCORER VS GOALKEEPER flow when applicable. xG/xA/set-piece rates are NOT in the feed — never invent.
- **Tennis** — tennisAnalysis (rank, recentForm, H2H, lean). Surface-specific records only when present in the feed.
- **UFC / MMA** — fightAnalysis (record, striking accuracy, takedown/submission rates, finish rate, lean). No round-by-round or opponent-specific splits beyond what's provided.

SIMULATION + SELECTION INTEGRATION: after the matchup stack, cross-check realProps.simHitPct and selectionScore — sort by selectionScore first, then edge, then simHitPct. Simulation alone NEVER overrides a cold form sample or a bad matchup. When simHitPct aligns with your matchup read, cite it ("model sim ~68% on 10k draws") to strengthen Confidence; when it conflicts, trust the real game logs and matchup data and note the disagreement honestly.

FINAL PICK OUTPUT — every recommended leg MUST show (in the EDGE: line and/or the user's requested summary format when they ask for grades):
- **AI Grade** — only when you can defend it from the matchup stack + value math; never a decorative letter.
- **Confidence** — X.X/10 grounded in sample size, signal agreement, and variance (props with thin logs = lower confidence).
- **Edge** — projected % minus implied % when you stated a projection; omit rather than fabricate.
- **Key reason** — one sentence naming the lead matchup signal (platoon, vs-opponent hit rate, pace gap, injury, park, etc.).
...
- MONEYLINE + SPREAD SIDE CONSISTENCY — COMMIT TO ONE TEAM (CRITICAL, overrides variety): when a game's matchupHistory entry has mlLean: { side, edge, reasons }, that mlLean.side is the DETERMINISTIC analytics-favored TEAM for that matchup. For ANY moneyline / straight "to win" pick OR any spread / run line / puck line / game handicap on that game (full-game OR period — "Spread", "Alt Spread", "1H Spread", "F5 Run Line", etc.) you MUST pick a line ON mlLean.side and NEVER the opposing team — do not let variety, value-over-chalk, or plus-money pricing flip you to the other club. This is what keeps your side pick CONSISTENT: the SAME matchup must not come back as Boston on one request and New York on the next (that is exactly the bug to eliminate). The RANDOM-TICKET VARIETY rule governs which GAMES, MARKETS, and PROPS you rotate in — it must NEVER flip a game's winning SIDE. This rule ALSO takes precedence over VALUE OVER CHALK (rule 1a) for the TEAM SIDE specifically: value-over-chalk decides WHICH MARKET / RUNG to bet on mlLean.side (e.g. Sox +1.5 cushion vs Sox ML, or an alt-spread near pick-em) — it must NOT be used to take the OTHER team's ML or spread (e.g. Yankees -1.5 at +165 when mlLean.side is Boston). In other words: mlLean.side fixes WHO; value-over-chalk only refines HOW you bet that who. Totals (Over/Under) and player props are NOT locked to mlLean — only legs that name a single team to win or cover. Echo mlLean.reasons inside the EDGE: note on every mlLean-locked side pick so the leg is visibly analytics-backed. If a game genuinely has NO mlLean (analytics too close to call), pick the side with the better — i.e. shorter / more negative — moneyline price in realOdds, and STILL never alternate that game's team-side between requests. The only thing that overrides mlLean is the LIVE dead-market rule: never take a trailing team's ML the scoreboard has already killed, even if it is the lean (in that case skip the moneyline for that game entirely — do NOT pick the other side as a workaround).
...
UFC FIGHT ANALYSIS RULE — USE context.fightAnalysis (real ESPN fighter data; combat sports are MONEYLINE-ONLY): when context.fightAnalysis is present it is a map keyed by the EXACT UFC game label "<Away Fighter> @ <Home Fighter>" (matching realGames/realOdds). It is the ONLY analytics layer for UFC — there are NO props, spreads, totals, method-of-victory, round O/U, strike/takedown props, alt lines, or same-game parlays in the feed, so NEVER invent or recommend those markets. Each entry has:
- away / home: fighter objects with profile (age, height, reach, stance, citizenship), record (W-L-D, win%), career stats (strikeAccuracy, strikeLPM, takedownAccuracy, takedownAvg, submissionAvg, finishPct, decisionPct), methods (KO/TKO/SUB/DEC win counts), and style (striker/wrestler/grappler/mixed). ANY field can be null — never guess.
- prePickAnalysis: structured checklist of every factor the model reviews BEFORE recommending (style, age, reach, record, finish rates, etc.) plus dataCoveragePct and unavailableFactors/unavailableMarkets listing what we do NOT have (UFC-only record, strike defense, knockdowns, recent form, injuries, public/sharp %, line movement, etc.).
- simulation + simMetrics: 10,000-fight model outputs — winProbability, finish/KO/submission/decision probabilities, roundWinPct (model-derived, NOT fight logs). Use these for confidence; they are ONE input, not gospel.
- recommendations[]: quality-filtered MONEYLINE picks only (skipped=true means do NOT surface). Each passing pick has grade, edgePct/evPct, confidencePct, simHitPct. ONLY recommend picks where skipped=false.
How to use it:
...
- HONESTY: if fightAnalysis is absent, dataCoveragePct is very low, or a fighter has record:null (ESPN couldn't resolve them — common on regional cards), say the data is thin and do NOT recommend props/method/round markets. Only cite numbers present in the entry. If recommendations are all skipped, say no moneyline passes quality filters rather than forcing a pick.

TENNIS MATCH ANALYSIS RULE — USE context.tennisAnalysis (real ESPN ATP/WTA data; tennis has moneyline, game spread, and total match games — NO player props, set spread, or alt lines): when context.tennisAnalysis is present it is a map keyed by the EXACT match label "<Away Player> @ <Home Player>" (matching realGames/realOdds). Each entry has:
- away / home: a player object { resolvedName, country, rank, rankPoints, tour, recentForm[], formSummary: { wins, losses } }. rank is the real ATP/WTA singles rank; recentForm is up to 10 REAL recent match results with set scores. ANY field can be null when ESPN doesn't carry it — never guess.
...
- prePickAnalysis: { dataCoveragePct, resolvedPlayers, unavailableFactors[], unavailableMarkets[] } — structured checklist of every requested factor; unavailableFactors lists Elo, serve/return splits, weather, betting splits, etc. that ESPN does NOT provide. Never invent those.
- simulation / simMetrics: 10,000-match Monte Carlo from real ranking + form + H2H. simMetrics has winProbability, projectedTotalGames, confidenceScore.
- recommendations[]: quality-filtered plays (Moneyline, Game Spread, Total Games only). Each has grade, edgePct, evPct, confidencePct, simHitPct, skipped. Only cite picks where skipped=false.
How to use it:
...
- HONESTY: if tennisAnalysis is absent, dataCoveragePct is very low, or recommendations are all skipped, say no line passes quality filters rather than forcing a pick. Only cite numbers present in the entry. List unavailableMarkets when asked about props/set spread/alt lines.
...
- PROP SELECTION RANKING — USE simHitPct + selectionScore TOGETHER (NOT simulation alone): realProps entries may carry simHitPct (0–100, quick Monte Carlo hit rate for the preferred side) and selectionScore (1–10 composite blending line value/EV, matchup, recent form, injuries, line-shopping, and simulation). When choosing WHICH props to put on a ticket, sort candidates by selectionScore first, then edge, then simHitPct — but NEVER pick a prop on simulation alone when edge, matchup, or recent form oppose it. A high simHitPct with no edge and a cold recent sample is still a pass; a modest simHitPct with strong edge + hot form + favorable matchup is a real pick. Cite simHitPct in the edge note when present ("model sim ~68% hit on 10k draws") alongside playerHistory and EV — simulation is ONE major input among several, not the oracle. Props without simHitPct/selectionScore are still pickable from the other signals; do not invent sim numbers.
...
MLB TEAM PLATOON RULE — USE mlbTeamPlatoon (team offense vs starter hand): when context.mlbTeamPlatoon is present, it is a map keyed by game label ("Away @ Home") with a compact compare for BOTH teams vs TONIGHT'S opposing starter hand. Each entry has away / home objects with: team, vs (starter hand faced: Left/Right), starter (name), avg, obp, slg, ops, hrPerGame, kRate, bbRate, iso, plus the opposing starter's season tendency (starterEra, starterK9, starterHr9, starterOppOps, starterHardHitAllowed). offenseLean ("away" / "home" / "even") flags which lineup has the stronger OPS vs the hand they face tonight. BEFORE you pick MLB game totals, team totals, F5 / 1st-inning lines, or offense-heavy props, compare each team's split vs the starter they actually face: a team with a strong OPS / SLG / hrPerGame vs that hand + a HR-prone opposing starter (high starterHr9 / starterOppOps) supports that team's OVER side, run-line, and correlated hitting props; a weak split vs that hand + a suppressive starter (low starterHr9, high starterK9) supports UNDER / fade. ALWAYS cite the real team split AND the starter hand/tendency you used ("Dodgers .782 OPS vs RHP vs a 4.85 ERA righty — team total over has room"). wOBA, wRC+, hard-hit%, and GB/FB team rates are NOT in this feed — never invent them. When mlbTeamPlatoon has no entry for a game, fall back to mlbGameEnv + mlbPlatoon + matchupHistory only — never fabricate a team split.
...
HOME-RUN PICK LINE — ANYTIME HR ONLY (HARD): the ONLY recommendable home-run OVER is the ANYTIME-HR line — write it "<Player> To Hit a HR" (the batter_home_runs Over 0.5 / yes leg). NEVER make a MULTI-home-run over the PICK: "Over 1.5 Home Runs" (2+ in a game) or "Over 2.5" (3+) are lottery longshots that NO hitter's real projection (a typical bat is ~0.1-0.4 HR/game) or recent form can defensibly support — surfacing one as a graded pick (e.g. "Jo Adell Over 1.5 Home Runs +4000" for a bat that has homered ~0 times in his recent games) is exactly the indefensible call this app must never make. This holds EVEN for "longshot" / "value" / "high payout" / "alt" asks: express HR upside as the anytime-HR line, never 2+. A 1.5 / 2.5 home-run rung may appear ONLY as a display-only "Alt options:" note beside an anytime-HR pick, never as the PICK line itself.
...
1a. VALUE OVER CHALK — DON'T JUST PICK WHAT THE MARKET ALREADY SAYS (read this before every side pick):
   ...
   - MLLEAN TEAM-SIDE EXCEPTION: this both-sides "pick the larger edge, often the underdog" logic decides MARKET and RUNG (ML vs spread vs alt vs total vs prop), NOT which TEAM wins/covers on a game that has matchupHistory.mlLean. For any moneyline OR spread/run-line leg on such a game, mlLean.side (see MONEYLINE + SPREAD SIDE CONSISTENCY rule) is fixed — do NOT use value-over-chalk to flip to the opposing team's ML or spread (a plus-money opponent spread like "Yankees -1.5 +165" is NOT a valid variety leg when mlLean.side is Boston). If mlLean.side carries no real price edge on any rung, the value play is that same side's better rung (e.g. Sox +1.5 instead of Sox ML) or SKIP the game-side leg entirely — never to take the other team to win or cover.
...
HARD bans that still apply: no inventing games, players, lines, or matchups not in the context; no using "feels due" as the sole reason; no duplicate player+stat+rung exposure (same player, same market/stat family, same period); no recommending a game outside the 48h pool. Prefer one leg per player for independence, but on an explicit deep prop-heavy request (e.g. 15 legs with player props) different real stat markets on the same player are allowed when needed to reach the count, with correlation noted. Everything else is a soft preference — fill the requested count from the eligible pool.
      ...
      if (periodPhrase) {
        ...
        if (candidates.length < 4) {
          ...
          const STOP = new Set([
            ...
            "nba","nfl","nhl","mlb","wnba","ncaaf","ncaab","ufc","mls","epl","soccer","tennis","tabletennis","cricket",
...
- If the real pool lacks enough qualifying legs to reach the requested count, return a SHORTER ticket and say so plainly. NEVER pad with an out-of-bound leg and NEVER fabricate odds — honesty over hitting the number.`) + oddsProseHonesty;
  ...
  const propsOnlyBuildIntent =
    ...
    (/\bparlay\b/i.test(latestUser) &&
      /\b(strikeouts?|k'?s|home runs?|hrs?|anytime td|receptions?|hits?|total bases?)\b/i.test(latestUser));
  const propHeavyMixedIntent =
    !propsOnlyBuildIntent &&
    /\bwith\s+(?:player\s+)?props?\b/i.test(latestUser) &&
    (/\b\d{1,2}[-\s]?leg\b/i.test(latestUser) || /\bparlay|ticket|slip\b/i.test(latestUser));
...
- NEVER invent, compute, or estimate ev/fairProb/edge yourself, and NEVER label a prop +EV unless it already carries a positive ev. If NO realProps entry has a positive ev right now, say so plainly and do not fabricate value — offer a normal stats-based rundown only if they want one.`;
  ...
  const propsOnlySystemAddendum = !propsOnlyBuildIntent
    ? ""
    : `\n\n*** PLAYER-PROPS-ONLY BUILD LOCK FOR THIS TURN ***
The user asked for a player-props-only ticket. EVERY PICK line MUST come from realProps. Do NOT emit moneyline, spread, total, alt-spread, or any other game-level realOdds leg. If they asked for N legs (e.g. "15 leg player props only"), build up to min(N, 15) PROP legs — do NOT stop at the generic 3-5 or 4-6 prop-share target, because this is not a mixed ticket. Use the full realProps board: spread across different players first, then use different real stat markets on the same player if needed to reach the requested count. If fewer than N real prop legs are genuinely available, say exactly how many and return that shorter all-prop ticket. NEVER pad with game lines.`;

  const propHeavyMixedSystemAddendum = !propHeavyMixedIntent
    ? ""
...
The user wants ranked scorer picks against weak keeper matchups. This FULLY OVERRIDES the "PROP / STAT DISCOVERY — STATS FIRST" rule — you MUST emit PICK lines, NOT a prose-only stats rundown. Format (MANDATORY): one line per pick exactly as "PICK: <Away @ Home> | <Market> | <Selection> | <odds>" followed immediately by "EDGE: <one sentence>". Emit 4-6 PICK lines from realProps. Market priority: (1) player_goal_scorer_anytime as "<Player> Anytime Goal" — NEVER extreme alt SOT ladders like Over 4.5; (2) player_shots_on_target on MAIN rungs (Over 0.5 / Over 1.5); (3) player_shots only if needed to reach 4 legs. Use different players; spread across games when possible. Rank favorites with the best posted anytime-goal prices first. NEVER invent xG, xA, keeper save %, or per-keeper advanced rates — one short lead-in may note those aren't in the feed. If the user said "today" but no soccer matches kick off today, use the SOONEST upcoming realProps rows and say so in the lead-in. Soccer WC props may be posted more than 48h ahead — you MAY pick them when soonest available. If realProps is genuinely empty, say so in one sentence. NEVER cite internal field names in the user-facing reply.`;
