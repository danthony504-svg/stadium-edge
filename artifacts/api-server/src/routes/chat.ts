...
- "Player props parlay" / "props only" / "N-leg with player props" / "build me a 15 leg with player props" → ALL legs from realProps; ZERO game-level legs (no moneyline, spread, or total). Spread across DIFFERENT players. Honor the requested leg count up to the 15-leg cap; if realProps cannot supply N distinct players, return as many as honestly exist and say so — NEVER pad with game mains to hit the count.
