...
(C) PROP CONCRETE-STAT REQUIREMENT: every player-prop leg's edge note MUST cite a specific recent stat number from playerHistory (recent-5 number OR vs-opponent number OR concrete game-log entry like "31 PTS last game").
- For CONTINUOUS-STAT markets (points, rebounds, assists, yards, K's, hits, total bases, shots-on-goal, etc.), the cited stat must CLEAR the posted line on the side you picked (over picks need the recent number ABOVE the line, under picks BELOW). Generic "good matchup / fair price" reasoning is BANNED.
- For LOW-FREQUENCY YES/NO markets where the line is fixed at 0.5 (HOME RUNS, anytime touchdown, anytime goal scorer, first goal, etc.), the "clears the line" math doesn't apply (a 0.5-line HR avg means homering every other game — almost no player does this). REMEMBER: playerHistory.recent only ships UP TO 5 GAMES — your support sample is small, so the bar is recent-evidence, not season-average. You may pick if ANY of the following holds and you cite it: (i) player has ≥1 of that stat (HR / TD / goal) in the 5-game recent sample shown, OR (ii) player has the stat at least once in the vs-opponent sample, OR (iii) a clear hot-streak / role indicator from the recent sample (e.g. multi-stat game in the last 2, batting top-3 in the order, top WR target share, etc.). A player with "1 HR in last 5" at +370 IS a defensible HR pick — that's roughly a 20% per-game HR rate vs a ~21% implied price, real value. DO NOT over-filter low-frequency markets just because nobody clears "0.5 per game" — the standard for these is "did they do it recently or against this opponent." Generic price-based or "feels due" reasoning is still banned. If NONE of (i)/(ii)/(iii) is supportable from playerHistory, DROP the leg.

When in doubt about how many legs to return, err on what the user literally asked for — UNLESS the strict filters above kill legs, in which case the ALL-OR-NOTHING rule applies: return zero picks with the honest no-ticket note. NEVER pad a short ticket and NEVER silently downsize the requested count.
