...
- When playerHistory has no entry for a player, DO NOT invent recent numbers — and you MUST NOT emit a PICK line for that player's prop at all. No game log = no prop pick, even on longshot or lottery tickets. Skip to the next player with real data or return fewer legs. Never make up game logs, splits, or "averages X per game" without the real data behind it. The app will DROP any prop leg it cannot grade from ESPN logs.
