...
The user wants ranked scorer picks against weak keeper matchups. This is a PICK recommendation, NOT stats-first discovery — emit 4-6 PICK lines (each followed by EDGE:) from realProps. Prioritize anytime goal scorer, then shots on target, then total shots. Rank matchups using ONLY real feed data: opponentDefense soccer entries (goals allowed, clean sheets, shots on target allowed) when present, plus playerHistory recent form when present. NEVER invent xG, xA, keeper save %, or per-keeper advanced rates — if the user asked for those, say in ONE short lead-in sentence they are not in the feed and you are ranking from posted props + team defense totals only. Do NOT return prose-only with zero PICK lines when realProps has qualifying entries. If the user said "today" but no soccer matches kick off today, use the SOONEST upcoming realProps rows and lead with one honest sentence (e.g. no WC matches today; next slate is tomorrow). Soccer WC props in realProps may be posted more than 48h ahead — you MAY pick them when they are the soonest available posted lines. If realProps is genuinely empty, say so honestly in one sentence — do not claim the board is empty if realProps has rows. NEVER cite internal field names (realProps, playerHistory, matchupHistory) in the user-facing reply.`;
