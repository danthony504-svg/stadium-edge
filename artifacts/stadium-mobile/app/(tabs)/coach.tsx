...
export default function CoachScreen() {
  ...
  const send = useCallback(
    async (
      ...
      if (openingParlayBuild) {
        ...
        if (seed?.boardScan?.picks?.length) {
          ...
          if (openingPicks?.length) {
            openingLegNote =
              earlyLegTarget > 0 && openingPicks.length < earlyLegTarget
                ? seed.boardScan.note ||
                  `You asked for **${earlyLegTarget}** legs — showing **${openingPicks.length}** from the precomputed slate while the full-board scan continues.`
                : seed.boardScan.note;
          }
        }
      }
      ...
      if (openingParlayBuild) {
        if (!tryInstantSlateSeedDelivery(earlyLegTarget, slateSport)) {
          startSlatePreAnalysis("coach-send", slateSeedOpts);
          await awaitWarmSlateSeed({
            legs: earlyLegTarget,
            sport: slateSport,
      ...
      try {
        ...
        if (replay) {
          ...
          if (reachFullPreScan) {
            ...
            mlbGameEnv: context.mlbGameEnv,
              ...
              const scanForDelivery = preferBoardScanForDelivery(
                preBoardScan,
                latestBoardScanRef.current,
              );
              if (scanForDelivery) {
          ...
          try {
            if (skipModelStreamForBoardScan || useParlayKernel) {
              full = "";
              setWaiting(false);
              let scanForDelivery = preferBoardScanForDelivery(
                preBoardScan,
                latestBoardScanRef.current,
              );
              if (
                useParlayKernel &&
                !boardScanIsComplete(scanForDelivery) &&
                earlyReachBoardScanRef.current
              ) {
                try {
                  const earlyScan = await earlyReachBoardScanRef.current;
                  scanForDelivery = preferBoardScanForDelivery(earlyScan, scanForDelivery);
                } catch {
                  scanForDelivery = preferBoardScanForDelivery(
                    latestBoardScanRef.current,
                    scanForDelivery,
                  );
                }
        ...
        if (reachBoardEligible) {
          const cachedBoardScan = preferBoardScanForDelivery(
            preBoardScan,
            latestBoardScanRef.current,
          );
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [messages, streaming, buildFinishing, waiting, deliverBoardScanTicket, patchInstantBoardScanTicket, tryInstantSlateSeedDelivery]);
