...
type UIMessage = {
  ...
  /** Coach parlay build request id — isolates result state across sends. */
  coachRequestId?: string;
  /** True when matchup/form/injury context is still thin on committed cards. */
  coachResultPreliminary?: boolean;
  coachResultOutcome?: import("@/lib/coachResultState").CoachResultOutcome;
};
...
/** Whole-request watchdog — finalize or surface no-picks instead of spinning forever. */
const COACH_REQUEST_WATCHDOG_MS = 10_000;
...
export default function CoachScreen() {
  ...
  const requestWatchdogTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const clearCoachRequestWatchdog = useCallback(() => {
    if (requestWatchdogTimerRef.current) {
      clearTimeout(requestWatchdogTimerRef.current);
  ...
  const commitCoachFinalTicket = useCallback(
    (
      ...
      if (isCoachResultRequestStale(requestId, activeRequestIdRef.current)) {
        return false;
      }
      const partial = latestBoardScanRef.current;
      ...
      setMessages((prev) => {
        ...
        for (let i = copy.length - 1; i >= 0; i--) {
          if (copy[i].role === "assistant") {
            const applied = applyCoachResultToAssistantMessage({
              prev: copy[i] as import("@/lib/coachResultState").CoachAssistantMessageFields,
              requestId,
              picks,
              legTarget,
              legNote,
        ...
        const recovered = applyCoachResultToAssistantMessage({
          ...
          recovered ?? {
            ...
            legNote:
              buildCoachSuccessLegNote(
                picks.length,
                legTarget,
                ticketContextPreliminary(picks).preliminary,
              ) || undefined,
    ...
    [captureFromCoach, clearBuildStallWatchdog, clearCoachRequestWatchdog, scrollToEnd],
  );
  ...
  const commitCoachFinalTicketEmpty = useCallback(
    (opts: {
      ...
      if (isCoachResultRequestStale(opts.requestId, activeRequestIdRef.current)) {
        return;
      }
      setMessages((prev) => {
        const copy = [...prev];
    ...
    [clearBuildStallWatchdog, clearCoachRequestWatchdog, scrollToEnd],
  );
  ...
  const runFinalizeCoachTicket = useCallback(
    (partial: FullBoardScanResult, phase = "correlating"): boolean => {
      ...
      if (!boardScanIsComplete(partial) && phase !== "request-watchdog") {
  ...
  const armCoachRequestWatchdog = useCallback(
    (sendGen: number, requestId: string) => {
      clearCoachRequestWatchdog();
      requestWatchdogTimerRef.current = setTimeout(() => {
        if (sendGenerationRef.current !== sendGen) return;
        if (finalizedRequestIdRef.current === requestId) return;
        if (finalizeInFlightRef.current === requestId) return;

        const partial = latestBoardScanRef.current;
        const candidates = partial?.picks ?? [];
        const legTarget =
          activeRequestLegTargetRef.current ||
          partial?.requestedLegs ||
          requestedLegCount(activeParlayAskRef.current) ||
        ...
        console.log(
          "[coach-watchdog] request-timeout",
          JSON.stringify({
            requestId,
            sendGen,
            candidateCount: candidates.length,
            scanComplete: partial ? boardScanIsComplete(partial) : false,
            reason: candidates.length
              ? "finalize-highest-ranked-fallback"
              : "no-candidates",
          }),
        );

        if (candidates.length && partial) {
          const scanForFinalize = boardScanIsComplete(partial)
            ? partial
            : { ...partial, scanComplete: true, requestId, requestedLegs: legTarget };
          latestBoardScanRef.current = scanForFinalize;
          runFinalizeCoachTicket(scanForFinalize, "request-watchdog");
          return;
        }
      ...
      if (legTarget >= 3) {
        ...
        if (!finalized.ok) {
          ...
          ticket = salvaged.length
            ? salvaged
            : coerceCoachDisplayPicks(partial.picks, enrichWithScan).slice(
                0,
                legTarget > 0 ? legTarget : partial.picks.length,
              );
        } else {
  ...
  const send = useCallback(
    async (
      ...
      if (openingParlayBuild && earlyLegTarget >= 3) {
        ...
        bindCoachResultRequest(varietySeed, earlyLegTarget);
        advanceCoachPhase("loading-markets", true);
        armCoachRequestWatchdog(sendGen, varietySeed);
      }
      ...
      try {
        ...
        if ((salvageBuilt || boardBuilt || soccerScorerGkSalvage) && picks.length > 0) {
          ...
          const streamRequestId = coachRequestContextRef.current?.requestId ?? null;
          const deferCoachEmptyForHandoff =
            isParlayBuild &&
            legTarget >= 3 &&
            !canPublishCoachNoPicksResult(streamRequestId);
          if (!deferCoachEmptyForHandoff) {
        ...
        const gateResolvedPicks = (resolved: ParsedPick[]): ParsedPick[] => {
          ...
          const salvaged = resolved.slice(0, legTarget > 0 ? legTarget : resolved.length);
          if (salvaged.length) return salvaged;
          const scanCandidates = fullBoardScanMeta?.picks ?? latestBoardScanRef.current?.picks;
          if (scanCandidates?.length) {
            return salvageHighestRanked(scanCandidates, ticketEnrich, legTarget);
          }
          return [];
        };
        ...
        const alreadyFinalized =
          !!requestId && finalizedRequestIdRef.current === requestId;
        const boardScanDone =
          freshBoardScanComplete &&
          !!fullBoardScanMeta &&
          boardScanIsComplete(fullBoardScanMeta);
        ...
        if (!alreadyFinalized && !(boardScanDone && isParlayBuild && legTarget >= 3)) {
        setMessages((prev) => {
          const copy = [...prev];
                  ...
                  <View style={{ gap: 8, marginTop: 10 }}>
                    {showTicketPicks || hasScanManifest ? (
                      <CoachTicketHeader
                        ...
                        preliminary={m.coachResultPreliminary}
                      />
                    ) : null}
