...
export default function FantasyStartSitScreen() {
  ...
  const compare=async()=>{if(!playerA||!playerB)return;const releaseOtaBlock=blockOtaReload();try{const [a,b,inj]=await Promise.all([getFantasyNflPlayerHistory(playerA.athleteId),getFantasyNflPlayerHistory(playerB.athleteId),getInjuries("nfl")]);const aa=historicalFantasyAnalysis(a.games,defaultRoster.scoringFormat).recentAverage,bb=historicalFantasyAnalysis(b.games,defaultRoster.scoringFormat).recentAverage;const injured=new Set(inj.flatMap(t=>t.entries.filter(e=>!/active|healthy/i.test(e.status)).map(e=>e.player.toLowerCase())));setResult(aa==null||bb==null?"INSUFFICIENT DATA":injured.has(playerA.name.toLowerCase())||injured.has(playerB.name.toLowerCase())?"TOO CLOSE":aa>bb+1?"START PLAYER A":bb>aa+1?"START PLAYER B":"TOO CLOSE");}finally{releaseOtaBlock();}};
