...
export function propHasGroundedGameLog(
  ...
  if (!mk) return false;
  return groundedPropStatGames(ph, mk, minGames) >= minGames;
}
...
export function groundedPropHistoryNote(dropped: ParsedPick[]): string {
  ...
  return `_Dropped ${dropped.length} player prop${dropped.length === 1 ? "" : "s"} with no real game-log sample for that stat${label} — the Coach only recommends props it can grade from real ESPN logs._`;
}
