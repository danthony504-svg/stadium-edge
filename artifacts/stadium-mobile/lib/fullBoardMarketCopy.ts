...
export function fullBoardScanShortfallNote(
  ...
  const staged =
    staging && (staging.mainOnTicket > 0 || staging.altOnTicket > 0)
      ...
      : "";
  return `_Scanned the entire board — **${totalScanned}** posted lines across ${FULL_BOARD_MARKET_FAMILIES} (10k sim each, cross-book line shopping, correlation scoring, and historical learning applied). **${staging?.mainQualified ?? totalQualified}** main lines and **${staging?.altQualified ?? 0}** alt lines cleared their quality bars (main: sim + edge + EV + grade ≥ C+ + confidence ≥ 52%; alt: positive EV/edge, grade ≥ C+, confidence ≥ 50%).${staged} Here are the top **${pickCount}** by EV, edge, confidence, and AI grade._`;
}
