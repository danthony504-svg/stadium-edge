...
import FantasyStartSitScreen from "../app/fantasy-start-sit";

const mockBack = jest.fn();
const mockSearchPlayer = jest.fn(async () => ({ results: [{ athleteId: "b", name: "Player B", position: "WR", team: "SEA", sport: "nfl" }] }));
const mockPill = (_: { label: string; onPress: () => void }) => <React.Fragment />;
...
jest.mock("@/components/ui", () => ({ Card: "Card", FONT: { display: "display", body: "body", bold: "bold", semibold: "semibold", medium: "medium" }, Pill: mockPill }));
...
test("Start/Sit has readable controls, selects Player B, and returns to the team", async () => {
  ...
  expect(screen.root.findAll((node) => node.props.accessibilityLabel === "Search players").length).toBeGreaterThan(0);
